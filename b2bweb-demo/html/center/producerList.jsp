                ...
                <form class="form-inline" role="form" style="margin:10px;">
					<div class="form-group">
						<label class="sr-only" for="keyword">关键词</label> <select
							...
							<option value="time">加入时间(年份)</option>
...
<c:forEach var="mkey" items="${key}">
	<script>	  
  		...
  		var teststr="";  
  		var testsh="";
		if(test=='14111810315435972459') {teststr="入驻"; testsh="<a href=\"<%=basePath%>purchase/viewProducer?id=${mkey['id']}\">查看</a>";}
		
		if(test=='14111918085389726019') {teststr="未入驻"; testsh="<a href=\"<%=basePath%>purchase/showProducer?id=${mkey['id']}\">审核</a>";}
		var arr  =
		     {   
		         ...
		         "address" :"${mkey['address']}",
		     	 ...
		     	 "state":teststr,
		     	 "bj":testsh,
		     	 "sc":"<a href=\"<%=basePath%>purchase/viewProducer?id=${mkey['id']}\" onclick=\"return delList();\"></a>"
		     	 	};
		jsonarray.push(arr);
	</script>
</c:forEach>
...
<script>
	...
	function ssfun() {
		...
		if (selectcont == 0) {
			...
			var	trHtml = "<tr><td>"+ jsonarray[i].index + "</td><td>"
						+ jsonarray[i].name + "</td><td>" + jsonarray[i].address
						+ "</td><td>" + jsonarray[i].addtime + "</td><td>"+jsonarray[i].state+"</td><td>"
						+ jsonarray[i].bj 
       ...
       if (selectcont == 2) {
			...
			for ( var i = 0; i < jsonarray.length; i++) {
					var	trHtml = "<tr><td>"+ jsonarray[i].index + "</td><td>"
						...
						+ jsonarray[i].bj 
						+ jsonarray[i].sc + "</td></tr>";
				if ((jsonarray[i].addtime.slice(0,4)).indexOf(keywordValue) >= 0) {
					if (buffercount < 10)
						$("#dpgltable").append(trHtml);
					databuffer.push(trHtml);//存入数据到缓存
					buffercount++;
	...
	function chg() {
		...
		if(document.getElementById("sel").value == "time"){
		selectcont = 2;
		}
	}
</script>
...
<script>
$(function(){
	  $('#centerProducer').css("color","#428BCA"); 
	 });
</script>
