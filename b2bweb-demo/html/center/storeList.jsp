				...
				<form class="form-inline" role="form" style="margin:10px;"
					...
					<div class="form-group">
						<label class="sr-only" for="keyword">关键词</label> <select
							...
							<option value="txt">店铺名称</option>
							<option value="btn">店铺地址</option>
...
<script type="text/javascript">
 function delAll(){//删除全选的数据
            ...
            success: function() {
               alert("删除成功！");
               window.location.href="<%=basePath%>purchase/consigneeAddressList";
             },
            error: function() {
            _this.text('请求失败');
            }
...
<c:forEach var="mkey" items="${key}">
	<script>	
		...
		if(test=="未审核"){ teststr="未审核";teststr1="<a href=\"<%=basePath%>purchase/editStoreList?id=${mkey['pid']}\">审核</a>"; }
		if(test=="审核通过"){ teststr="审核通过";teststr1="<a href=\"<%=basePath%>purchase/editStoreList?id=${mkey['pid']}\">查看</a>";}
		if(test=="未通过"){ teststr="未通过";teststr1="<a href=\"<%=basePath%>purchase/editStoreList?id=${mkey['pid']}\">审核</a>";}
		var arr  =
		     {
				 "index" :"<input type=\"checkbox\" class=\"ck-item-box\" id=\"checkboxId\" name=\"checkboxId\" value=\"${mkey['id']}\">",
		         "companyName" : "${mkey['name']}",
		         "adress" : "${mkey['provinceName']}-${mkey['cityName']}-${mkey['regionName']}",
		     	 "buildDate":"${mkey['addtime']}",
		     	 "state":teststr,
		     	 "bj":teststr1,
		     	 "sc":"<a href=\"<%=basePath%>purchase/delStoreList?id=${mkey['id']}\" onclick=\"return delList();\">删除</a>"
		     	 	};
		jsonarray.push(arr);
	</script>
</c:forEach>
...
<script>
		...
		if (selectcont == 0) {
			for ( var i = 0; i < jsonarray.length; i++) {
				...
				var trHtml = "<tr><td>"+ jsonarray[i].index + "</td><td>"
						+ jsonarray[i].companyName + "</td><td>" + jsonarray[i].adress
						+ "</td><td>" + jsonarray[i].buildDate + "</td><td>"
						+ jsonarray[i].state + "</td><td>" + jsonarray[i].bj
						+ jsonarray[i].sc + "</td></tr>";
				if ((jsonarray[i].companyName).indexOf(keywordValue) >= 0) {
					if (buffercount < 10)
			...
			for ( var i = 0; i < jsonarray.length; i++) {
				var trHtml = "<tr><td>"+ jsonarray[i].index + "</td><td>"
						...
						+ jsonarray[i].state + "</td><td>" + jsonarray[i].bj
						+ jsonarray[i].sc + "</td></tr>";
				if ((jsonarray[i].adress).indexOf(keywordValue) >= 0) {
					if (buffercount < 10)
					$("#dpgltable").append(trHtml);
					databuffer.push(trHtml);//存入数据到缓存
					buffercount++;
	...
	function chg() {
		if (document.getElementById("sel").value == "txt") {
			selectcont = 0;
		} else if(document.getElementById("sel").value == "btn") {
			selectcont = 1;
		}
	}
</script>
