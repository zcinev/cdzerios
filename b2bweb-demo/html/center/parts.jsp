...
<c:forEach var="mkey" items="${key}">
	<script >	
  		...
  		if(stateName !="审核通过"){
  		  testsh="<a href=\"<%=basePath%>purchase/editPartsList?id=${mkey['id']}&number=${mkey['autopartinfo']}\">审核</a>";
  		  }
  		 if(stateName=="审核通过"){
  		 testsh="<a href=\"<%=basePath%>purchase/editPartsList?id=${mkey['id']}&number=${mkey['autopartinfo']}\">查看</a>";
  		 }
		var arr  =
		     {
