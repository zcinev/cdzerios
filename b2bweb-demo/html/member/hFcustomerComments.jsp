...
<c:forEach var="mkey" items="${key}">
 ...
 <script type="text/javascript">
// 回复评论 显示页面内容
	function peiJIanCommentsLookReply(id){
 	          
 	     $.ajax({
		     type : "POST",
		     url : "<%=basePath%>member/peiJIanCommentsLookReply",
		     dataType : "json",
			 async : true,
		     data : {'id':id},
		     success : function(data){
				...
				for(var i=0; i<data.length; i++){
		     		...
		     		document.getElementById("nameId").value=data[i].nameId; 
 	...
 	var CommentReply=document.getElementById("CommentReply").value;
  	var nameId=document.getElementById("nameId").value;  
  	   alert(nameId);         
 	      $.ajax({
            type: "GET",
            data:{'id':nameId,'content':CommentReply},
            url: "<%=basePath%>member/peiJIanCommentsReply",
            async:false,
            dataType: "html",
            success: function(data) {
            alert("发送成功！");
