...
<c:forEach var="mkey" items="${key}">
	<script>	
  		...
  		var teststr="";  
  		var testsh="<a  href=\"<%=basePath%>pei/detail?id=${mkey['productId']}\" title='查看配件详情' target='blank'>查看</a>";
 		var arr  = 
		     {   
		         "index" : index,
		         "nichen" : "${mkey['reviewerName']}",
...
<script>
	...
	function ssfun() {
		...
		if (selectcont == 0) {
			for ( var i = 0; i < jsonarray.length; i++) {
				...
				var partsNameStr=jsonarray[i].partsName;
				  if(partsNameStr.length>10){
				  partsNameStr=partsNameStr.substring(0,10)+"...";
				  }
				
				trHtml.innerHTML = "<td>"
						+ jsonarray[i].index + "</td><td>"
						 + jsonarray[i].nichen + "</td><td title='"+jsonarray[i].partsName+"'>"
						 +partsNameStr  +  
						"</td><td ><span class='vote-star'><i style='width:"+jsonarray[i].rank+"'></i></span></td><td style='max-width:100px;white-space: nowrap;text-overflow:ellipsis;overflow:hidden;text-align:left;'>"
			...
			for ( var i = 0; i < jsonarray.length; i++) {
				...
				trHtml.innerHTML = "<td>"
						...
						"</td><td ><span class='vote-star'><i style='width:"+jsonarray[i].rank+"'></i></span></td><td style='max-width:100px;white-space: nowrap;text-overflow:ellipsis;overflow:hidden;text-align:left;'>"
						 ...
						 + jsonarray[i].createTime
						+ "</td><td>" + jsonarray[i].sc + "</td>";
				if ((jsonarray[i].partsName).indexOf(keywordValue) >= 0) {
					if (buffercount < 10)
						dpgltable.appendChild(trHtml);
					databuffer.push(trHtml.innerHTML);//存入数据到缓存
					buffercount++;
		...
		var trbuffer = "<td>"
						+ jsonarray[i].index + "</td><td>"
						 + jsonarray[i].nichen + "</td><td title='"+jsonarray[i].partsName+"'>"
						  +partsNameStr  +  
						"</td><td ><span class='vote-star'><i style='width:"+jsonarray[i].rank+"'></i></span></td><td style='max-width:100px;white-space: nowrap;text-overflow:ellipsis;overflow:hidden;text-align:left;'>"
						 + jsonarray[i].content+ "</td><td>"+ jsonarray[i].createTime
						+ "</td><td>" + jsonarray[i].sc + "</td>";
	...
	function chg() {
		...
		if (document.getElementById("sel").value == "pj1") {
			selectcont = 2;
		}
	}
