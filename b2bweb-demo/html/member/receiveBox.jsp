						...
						<tr>
							 <td colspan="6" style="text-align:left;"><span class="ck-all-box btn btn-default">全选</span>
								<button type="submit" class="delete-all-box btn btn-primary"
									onclick="return delAll();" >标记为已读</button>
							</td>
						</tr>
					</tbody>  
                    <tr>
	...
	//确定回复
 	function receBoxCommentReply(){
  	var CommentReply=document.getElementById("CommentReply").value;
  	           var recevier2=document.getElementById("sender2").value;
  	           alert(recevier2);
  	           
 	      $.ajax({
            type: "GET",
            data:{content:CommentReply,sender:recevier2},
            url: "<%=basePath%>member/receBoxCommentReply",
            async:false,
            dataType: "html",
...
function  delList(obj){
    ...
    if(con==true){
            ...
            type: "POST", 
            data:{'boxName':obj},
            url: "<%=basePath%>member/hasReadReceBox",
            async:false,
            dataType: "html",
            success: function(data) {
               location.reload();
...
<script type="text/javascript">
 function delAll(){
            ...
            dataType: "html",
            success: function() {
               window.location.href="<%=basePath%>member/receiveBoxTest";
             },
            error: function() {
            alert('请求失败');
            }
...
<c:forEach var="mkey" items="${key}">
	<script>	
  		...
  		var teststr = "<a  href='javascript:;' data-toggle='modal' data-target='#myModal' onClick='ReceListDetail(\"${mkey['id']}\")'>查看</a>";  
  		 var testsh="<a  href=\"javascript:void(0)\" onclick=\"delList('${mkey['id']}');\">删除</a>"; 
		var stName = "${mkey['stateName']}";
			...
			for ( var i = 0; i < jsonarray.length; i++) {
				...
				$(trHtml).html("<td>"
						+ jsonarray[i].index + "</td><td>"
						 + jsonarray[i].nichen + "</td><td style='max-width:200px;white-space: nowrap;text-overflow:ellipsis;overflow:hidden;text-align:left;'>"
...
<script type="text/javascript">
function ReceListDetail(obj){
  
	$.ajax({
	     type : "POST",
	     url : "<%=basePath%>member/receBoxDetailTest",
	     dataType : "json",
		 async : true,
	     data : {id:obj},
	     success : function(data){
