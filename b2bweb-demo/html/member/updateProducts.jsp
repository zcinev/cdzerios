			...
			<div class="panel panel-default a_panel" style="width:1100px">
  					...
  					<div class="form-group">
						...
						<div class="col-sm-10 form-group form-inline"
							...
							<select name="firstKind" id="firstKindId-box"
								...
								class="form-control">
							</select> <select name="thirdKind" id="thirdKind-box" class="form-control">
							</select>
				...
				<div class="form-group">
						...
						<div class="col-sm-10 car-model-list">
							<div class="clearfix" style="min-width:440px;float:left;width:auto";>
						...
						<div class="col-sm-8 form-inline" style="padding-left:15px;">
							<input type="text" name="name" id="name" value="${mkey.name }"
								class="form-control" placeholder=""onblur="validatelegalName('name');" maxlength="100">
								<span class="message" >产品名称不能为空[]</span>
                                                 <span class="messageShow" ></span> 
					...
					<div class="form-group">
						...
						<div class="col-sm-8 form-inline" style="padding-left:15px;">
							<input type="text" name="leaveFactoryNumber" id="leaveFactoryNumber" class="form-control"
								placeholder="请输入出产编号" onblur="validatelegalName('leaveFactoryNumber');" value="${mkey.leaveFactoryNumber}"
						...
						<label for="" class="col-sm-2 control-label">适配车型：</label>
 						 ...
 						 <div class="col-sm-10 car-model-list" >
										<div class="clearfix">
											<a class="btn btn-info car-modal-btn">${mkey['brandName']}/${mkey['factoryName']}/${mkey['fctName']}/${mkey['speciName']}</a>
											<div class="hidden">
												 <input type="hidden" name="brandId"  value="${mkey['brand']}"> 
												 <input type="hidden" name="factoryId"  value="${mkey['factory']}">
												 <input type="hidden" name="fctId"  value="${mkey['fct']}">
												 <input type="hidden" name="speciId"  value="${mkey['speci']}">
												 <input type="hidden" name="biaoji"  value="0">
											</div>
												
											<span class="btn btn-info car-modal-plus-btn glyphicon glyphicon-plus"></span>
												<c:if test="${index=='1' }">
												<span id="modelidTestid"></span>
												</c:if>
							
											<c:if test="${index!='1' }">
											<span class="btn btn-info car-modal-delete-btn glyphicon glyphicon-trash"></span>
											</c:if>
										</div>
									</div>
										</div>
						...
						<div class="col-sm-8 form-inline" style="padding-left:15px;">
							<input id="factoryIdall" name="factoryIdall" type="hidden"
								...
								<select name="factory" id="manufacturer-select" class="form-control"
								 onblur="validatelegalName('factory');">
 							</select>
 							<span class="message" >产商不能为空[]</span>
                                                 <span class="messageShow" ></span>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-8 form-inline" style="padding-left:15px;">
							<input type="" name="marketprice" id="marketprice"
								value="${mkey.marketprice }" class="form-control"
								placeholder="市场价" onblur="validatelegalName2('marketprice');" oninput="priceList('${discount }');" maxlength="20"> 
								<span class="message" >市场价不能为空[]</span>
                                                 <span class="messageShow" ></span>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-8 form-inline" style="padding-left:15px;">
							<input type="" name="memberprice" id="memberprice"
					...
					<div class="form-group">
						...
						<div class="col-sm-8 form-inline" style="padding-left:15px;">
							<input type="text" name="stocknum" id="stocknum"
								value="${mkey.stocknum}" class="form-control" placeholder="添加数量"  onblur="javascript:addNumberList();" maxlength="20">
							<input style="position: absolute;left: 35%;top: 10px;border: 0px;" id="kucunNumber" value="库存(0)件" onblur="validatelegalName('stocknum');">
					...
					<div class="form-group">
						...
						<div class="col-sm-8 form-inline" style="padding-left:15px;">
							<input id="indextype" name="indextype" type="hidden"
								...
								id="indextypeId" class="form-control" onblur="validatelegalName('indextype');">
							</select>
							<span class="message" >产品类别不能为空[]</span>
                                                 <span class="messageShow" ></span>
						</div>
					</div>
					<%-- <div class="form-group" style="display: none;">
					...
					<div class="form-group">
						...
						<div class="col-sm-8 form-inline" style="padding-left:15px;">
							<input id="qualityId" name="qualityId" type="hidden"
								...
								id="guidsecurity" class="form-control" onblur="validatelegalName('guidsecurity');">
							</select>
							<span class="message" >产品质保不能为空[]</span>
                                                 <span class="messageShow" ></span>
						</div>
					</div>
	...
	<div class="modal fade car-model" style="margin-left: -200px;" id="car-model">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body" id="modalinfo" style="width:780px;height:400px;overflow:scroll">
...
function UpdatePeijianList(){ 
 ...
 var name=document.getElementById("name").value;					//产品名称
   var firstKindId=document.getElementById("firstKindId-box").value;	//配件一级分类
   var secondKindId=document.getElementById("secondKind-box").value;	//配件二级分类
   var thirdKindId=document.getElementById("thirdKind-box").value;	//配件三级分类
      var brandId=""; 
      var factoryId="";
      var seriesId="";
      var modalId="";
     ...
     var factory=document.getElementsByName("factoryId");		//厂商
      var series=document.getElementsByName("fctId");		//车型
       ...
       /*  for(var i=0;i<brand.length;i++){
        ...
        var factory=document.getElementById("factoryIdall").value;		//配件生产商
        var factory_id_0001=$('#manufacturer-select').val();//生产商id(2015-07-10 杨才文)
              ...
              var standard ="";
               ...
               var guidsecurityName=$("#guidsecurity option[value='" + guidsecurity + "']").text();
                ...
                var description=document.getElementById("faceImg3").value;
                 var indextypeId=document.getElementById("indextype").value;		//类别
	...
	if (!validatelegalName('stocknum')) {
	 ...
	 if (!validatelegalAndZero('factoryIdall')) {
		check = false;
	} 
...
$(function() {
         ...
         var _html = '<div class="clearfix"  style="min-width:440px;float:left;width:auto" > ';
...
obj1=obj;
  $.ajax({
		...
		document.getElementById("brandul").innerHTML+="<li onclick=\"showFactory('"+data[i].id+"','"+data[i].name+"')\"><img  src='"+data[i].imgurl+"'"+"/><span>"+data[i].name+"</span></li>";
		}
...
obj2=obj;
	  ...
	  $.ajax({
		...
		document.getElementById("brandul").innerHTML="";
		for(var i=0; i<data.length; i++){
		document.getElementById("brandul").innerHTML+="<li onclick=\"showFct('"+data[i].id+"','"+data[i].name+"')\"><span>"+data[i].name+"</span></li>";
		}
...
$('#cpinpai').html=buffer;
	  ...
	  $.ajax({
		...
		document.getElementById("brandul").innerHTML+="<li onclick=\"showSpeci('"+data[i].id+"','"+data[i].name+"')\"><img  src='"+data[i].imgurl+"'"+"/><span>"+data[i].name+"</span></li>";
...
$('#cpinpai').html=buffer;
	  ...
	  $.ajax({
		...
		document.getElementById("brandul").innerHTML+="<li style='padding-left:0px;margin-left: 5px;width:175px;' onclick=\"saveCarModel('"+data[i].id+"','"+data[i].name+"',this)\"><span style='font-size: 12px;'>"+data[i].name+"</span></li>";
		}
...
var index=$(thisElem11).parent().index();
            ...
            _html+= "<input type='hidden' name='speciId'  value='"+obj+"'></div>"; 
             _html+= " <span class='btn btn-info car-modal-delete-btn glyphicon glyphicon-plus' onclick=\"addInfo1(this);\"></span>";
...
function removeInfo1(thisElem){
	...
	if(k==0){
		...
		if(len2>1){
            ...
            /*  _html+= '<input type="hidden" name="biaoji1"  value="">'; */
            _html+= '<input type="hidden" name="speciId1"  value=""></div>'; 
            _html+= ' <span class="btn btn-info car-modal-delete-btn glyphicon glyphicon-plus"></span></div>';
       		alert(_html);
