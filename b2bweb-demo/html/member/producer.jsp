        ...
        --%><div class="col-md-12 pull-right paddingRight0 paddingLeft0">
            <div class="panel panel-default">
				...
				<form class="form-inline" role="form" style="margin:10px;"
					...
					<div class="form-group">
						<label class="sr-only" for="keyword">关键词</label> <select
							...
							<option value="txt">厂商名称</option>
							<option value="btn">厂商地址</option>
						</select> <input type="text" class="form-control" id="keyword"
							placeholder="请输入你要搜索的内容"> 
					</div>
					<button type="button" class="btn btn-primary" onclick="ssfun();" >搜索</button>
					 <span style="margin-left:385px"><a href='javascript:;'  onclick="addManufacturerList();"  style='text-decoration : underline; color: red;' >添加生产商</a></span>
  				</form>
                <table class="table table-striped" id="newtable">
					...
					<!-- <tbody>
						<tr>
							<td colspan="9"><input type="checkbox"> <em
								style="padding:0 10px; font-style:normal;"> <a href="#">全选</a>/<a
									href="#">反选</a> </em>
					...
					<tbody>
						<tr>
							 <td colspan="6"><span class="ck-all-box btn btn-default">全选</span>
							 
							 <button type="button" class="btn btn-primary" onclick="return delAll();" >删除选定</button>
							 </td>
						</tr>
					</tbody>
					   <tr>
...
window.location.href="<%=basePath%>member/addManufacturerList";
}	
</script>
<script type="text/javascript">
 function delAll(){
            ...
            url: "<%=basePath%>member/delfactoryListTest",
            async:false,
            dataType: "html",
            success: function() {
               alert("删除成功！");
               window.location.href="<%=basePath%>member/producerList";
             },
            error: function() {
            _this.text('请求失败');
            }
...
<c:forEach var="mkey" items="${key}">
	<script>	  
  		...
  		var testsh="";
  		var deletestr="";
		if(test==14111918085389726019) {
			teststr="未入驻"; testsh="<a href=\"<%=basePath%>member/updateManufacturer?id=${mkey['id']}\">编辑</a>";
			deletestr="<a href=\"<%=basePath%>member/delManufacturer?id=${mkey['id']}\" onclick=\"return delList();\" style='margin-left:8px;'>删除</a>";
		}
		if(test==14111810315435972459) {teststr="入驻"; testsh="<a href=\"<%=basePath%>member/viewManufacturer?id=${mkey['id']}\">查看</a>";}
 		var arr  =
		     {   
				 "index" :"<input type=\"checkbox\" class=\"ck-item-box\" id=\"checkboxId\" name=\"checkboxId\" value=\"${mkey['id']}\">",
		         "name" : "${mkey['name']}",
		         "address" : "${mkey['address']}",
		...
		if (selectcont == 0) {
			for ( var i = 0; i < jsonarray.length; i++) {
				...
				$(trHtml).html("<td>"  
						...
						+ jsonarray[i].address + "</td><td>"
						+ jsonarray[i].addtime+ "</td><td>"
						+ jsonarray[i].status+"</td><td>"
		...
		if (selectcont == 1) {
			for ( var i = 0; i < jsonarray.length; i++) {
				...
				$(trHtml).html("<td>"  
						...
						+ jsonarray[i].status+"</td><td>"
						+ jsonarray[i].bj + jsonarray[i].sc + "</td>");
				if ((jsonarray[i].address).indexOf(keywordValue) >= 0) {
					if (buffercount < 10)
						dpgltable.appendChild(trHtml);
					databuffer.push(trHtml.innerHTML);//存入数据到缓存
					buffercount++;
